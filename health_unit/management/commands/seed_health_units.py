...
class Command(BaseSeedCommand):
    ...
    def _create_health_units(self, fake, count):
        ...
        name_suffixes = [
            ...
            'I', 'II', 'III', 'IV', 'V',
            'Dr. José Silva', 'Dra. Maria Santos', 'Dr. João Oliveira',
            'Vila Nova', 'Centro', 'Jardim das Flores',
            'São José', 'Santa Maria', 'Nossa Senhora'
        ]
        
        addresses = list(Address.objects.all())
        if not addresses:
            self.stdout.write(
                self.style.ERROR('❌ Nenhum endereço disponível')
            )
            return 0
        ...
        for i in range(count):
            ...
            max_attempts = 5
            attempts = 0
            while HealthUnit.objects.filter(name=unit_name).exists() and attempts < max_attempts:
                suffix = fake.random_element(name_suffixes)
                unit_name = f"{unit_type} {suffix}"
                attempts += 1
            ...
            domain_options = [
                'saude.gov.br', 'prefeitura.sp.gov.br', 'sus.br',
                'saude.rj.gov.br', 'saude.mg.gov.br', 'saude.rs.gov.br'
            ]
