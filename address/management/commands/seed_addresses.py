...
class Command(BaseSeedCommand):
    ...
    def _create_addresses(self, fake, count):
        ...
        for i in range(count):
            ...
            while (
                Address.objects.filter(cep=cep, number=number).exists()
            ...
            address = Address(
                ...
                latitude=fake.pyfloat(
                    ...
                    right_digits=6,
                    positive=False,
                    min_value=lat_bounds[0],
                    max_value=lat_bounds[1],
                ),
                longitude=fake.pyfloat(
                    left_digits=2,
                    right_digits=6,
                    positive=False,
                    min_value=lon_bounds[0],
                    max_value=lon_bounds[1],
                ),
            )
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Erro ao criar endereços: {e}"))
            return 0
