...
class Command(BaseSeedCommand):
    ...
    def _create_users(self, fake, count):
        ...
        for i in range(count):
            ...
            user = User(
                ...
                address=fake.random_element(addresses) if addresses and fake.boolean(chance_of_getting_true=70) else None,
    ...
    def _create_patients(self, fake, users, count):
        ...
        for user in selected_users:
            ...
            patient = Patient(
                ...
                other_gender=fake.word() if fake.boolean(chance_of_getting_true=5) else None,
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Erro ao criar pacientes: {e}')
            )
            return []
    ...
    def _create_works(self, fake, users, count):
        ...
        if not health_units:
            self.stdout.write(
                self.style.WARNING('⚠️  Nenhuma unidade de saúde disponível para criar trabalhos')
            )
            return []
        ...
        for i in range(count):
            ...
            end_date = fake.date_between(start_date=start_date, end_date=start_date + timedelta(days=365*2))
            
            work = Work(
                user=user,
                health_unit=health_unit,
                permission_role=fake.random_element([choice[0] for choice in PermissionRole.choices]),
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ {len(works)} trabalhos criados!')
            )
            return works
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Erro ao criar trabalhos: {e}')
            )
            return []
    ...
    def _create_sessions(self, fake, users, count):
        ...
        for i in range(count):
            ...
            end_hour = start_hour + fake.random_int(min=1, max=8)  # 1 a 8 horas de trabalho
            if end_hour > 23:
                end_hour = 23
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Erro ao criar sessões: {e}')
            )
            return []
