...
class BaseSeedCommand(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--verbose-seed',
            action='store_true',
            help='Show detailed seed information'
        )
    ...
    def handle_seed(self, fake, *args, **options):
        ...
        """
        raise NotImplementedError("Classes filhas devem implementar handle_seed(fake, *args, **options)")
    ...
    def _print_success(self, result: Dict[str, Any]):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'✅ {self.seed_description} concluído!\n{stats_str}'
            )
        )
    
    def bulk_create_with_progress(self, model, objects, batch_size=1000, ignore_conflicts=True):
        """
