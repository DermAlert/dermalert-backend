...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--skip',
            nargs='+',
            help='Skip specific seeds during execution'
        )
    ...
    def _list_seeds(self):
        ...
        for i, seed_name in enumerate(execution_order, 1):
            ...
            deps_text = f" (depende de: {', '.join(dependencies)})" if dependencies else " (sem dependências)"
        ...
        if options.get('skip'):
            for seed in options['skip']:
                if seed in seeds_to_run:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f'⚠️  Pulando seed: {seed}')
                    )
    ...
    def _execute_seeds(self, seeds_to_run, options, seed_int):
        ...
        for i, seed_name in enumerate(seeds_to_run, 1):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ Erro ao executar {seed_name}: {e}'
                    )
                )
                raise
    ...
    def _prepare_seed_arguments(self, seed_name, options):
        ...
        if seed_name == 'seed_addresses':
            ...
            args['patients'] = min(options.get('users', 50), 30)  # Máximo de 30 pacientes
